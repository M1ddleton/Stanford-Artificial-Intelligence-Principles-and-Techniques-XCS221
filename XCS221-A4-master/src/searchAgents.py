...
# Wrong Corners problem
class WrongCornersProblem(search.SearchProblem):
  def __init__(self, startingGameState):
    self.walls = startingGameState.getWalls()
  ...
  def isGoalState(self, state):
    return True not in [state.hasFood(*c) for c in self.corners]
       
  def getSuccessors(self, state):
    self._expanded += 1
    return [(state.generateSuccessor(0, a), a, 1) for a in state.getLegalActions()]
...
def cornersHeuristic(state, problem):
  ...
  # 
  # note this expands ~ 774 nodes on mediumCorners
  pos = state[0]
  remaining = [c for i, c in enumerate(corners) if not state[1][i]]
  if len(remaining) == 0: return 0
  closest = min([util.manhattanDistance(c, pos) for c in remaining])
  farthest = max([util.manhattanDistance(c, pos) for c in remaining])
  allpairs = [(i,j) for i in remaining for j in remaining]
  maxManhattan = max([util.manhattanDistance(*pair) for pair in allpairs])
  #return farthest
  #return maxManhattan
...
def foodHeuristic(state, problem):
  ...
  #  ALT="return 0"
  return comboHeuristic(state, problem)
  return minManhattanDistance(state, problem)
  return allManhattanHeuristic(state, problem)
  return mstHeuristic(state, problem)
...
def allManhattanHeuristic(state, problem):
  """
  Sums the distances between each point (food or pacman) and its
  nearest neighbor.
  """
  foodList = state[1].asList()
  if(len(foodList) == 0):
    return 0
  foodList.append(state[0])
  totalCost = 0
  for x,y in foodList:
    minDist = 10000000
    for other_x, other_y in foodList:
...
def minManhattanDistance(state, problem):
  """
  Finds distance to the food nearest Pacman      
  """
  if state[1].count() == 0: return 0
  return min([util.manhattanDistance(state[0], p) for p in state[1].asList()])
      
def maxDistanceHeuristic(state, problem):
  """
  Finds the points (food or pacman) farthest from
  each other and uses that as the heuristic.
  """
  foodList = state[1].asList()
  ...
  for x,y in allList:
    maxDist = 0
    for other_x, other_y in allList:
      if (x,y) == (other_x, other_y):
        continue
      dist = abs(x - other_x) + abs(y - other_y)
...
def comboHeuristic(state, p):
  return max(numFoodHeuristic(state, p), maxDistanceHeuristic(state, p))
  return max(maxDistanceHeuristic(state, p), allManhattanHeuristic(state, p))
