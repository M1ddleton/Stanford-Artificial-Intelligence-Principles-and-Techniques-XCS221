...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            pass
            # ### START CODE HERE ###
            least_consistent_variable = None
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for val in ordered_values:
                        ...
                        if deltaWeight > 0:
                            count_a += 1
                    if count_a < least_count:
                        least_count = count_a
                        least_consistent_variable = var
            return least_consistent_variable
            # ### END CODE HERE ###
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                if len(request.cids) > 0:
                    csp.add_unary_factor((request, quarter),
                                         lambda cid: True if (cid is None or len(request.quarters) == 0 or (
                                                 quarter in request.quarters)) else False)
        # ### END CODE HERE ###
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    var = (cid, quarter)
                    quarter_minUnits = self.bulletin.courses[cid].minUnits
                    quarter_maxUnits = self.bulletin.courses[cid].maxUnits
                    csp.add_variable(var,
                                     list(range(quarter_minUnits, quarter_maxUnits + 1)) + [0])
                    new_var.append(var)
                    csp.add_binary_factor((request, quarter), var, lambda request_cid,
                                                                          course_unit: course_unit > 0 if request_cid == cid else course_unit == 0)
            quarter_sum = get_sum_variable(csp, quarter, new_var, self.profile.maxUnits)
